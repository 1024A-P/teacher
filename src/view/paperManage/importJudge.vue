  ...
  <div class="import-choice-form">
    <el-form ref="form" :model="form" label-width="80px">
      <el-form-item label="题目总分">
        <el-input v-model="form.judgeCount" placeholder="请输入题目总分" style="width:130px"></el-input> 分
      </el-form-item>
      <el-form-item label="题目概况">
        <el-input v-model="form.judgePoint" style="width:80px"></el-input>
        分 / 道，共 {{handlePoint()}} 道
      </el-form-item>
    ...
    <el-dialog
      ...
      <div class="choice-info-search">
        ...
        <div class="info-input">
          <wj-input v-model="judForm.txtId" label="题号" maxlength="30" placeholder="请输入题号" clearable></wj-input>
          <wj-input class="mgl20" v-model="judForm.txtName" label="题名" maxlength="30" placeholder="请输入题目名称" clearable></wj-input>
          <wj-select class="mgl20" v-model="judForm.hardType" label="难度" :options="hardOption"></wj-select>
        </div>
        <!-- 右侧按钮 -->
      ...
      <wj-table :tableData="judgeList" @selectionChange="getChoList" maxHeight="450px" :showPage="false">
...
export default {
  ...
  methods: {
    ...
    handlePoint () {
      let result = ''
      if (this.form.judgeCount !== '' && this.form.judgePoint !== '') {
        result = (this.form.judgeCount * 1) / (this.form.judgePoint * 1)
    ...
    getJudgeList () {
      ...
      this.$http.post('/api/manage/getJudgeList', data).then(res => {
        if (res.body.msg === 'success') {
          ...
          // 处理数据集
          this.judgeList.list = this.judgeAllList
        } else {
          this.judgeList.isloading = false
          this.judgeAllList = []
          this.judgeList.list = []
    ...
    nextStep () {
      if (this.choIdAll.length === this.topicNum) {
        this.form.judgeId = this.choIdAll
        this.$emit('thirdFinsh', this.form, this.choTagList)
      } else {
